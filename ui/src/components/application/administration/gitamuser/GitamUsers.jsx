...
const GitamUsers = (props, initialValues) => {
  ...
  const fetchserviceLists = () => {
    servicesaxios
      .get(`plans/v3/list?limit=10&page=1&status=ACT`)
      .then((response) => {
        const { data } = response;
        const { count, counts, next, previous, page, results } = data;
        setPlanList(results);
        console.log(results, "kumaraaaaaa");
      })
  ...
  useEffect(() => {
    ...
    adminaxios
      // .get(`accounts/users`)
      .get(`accounts/universityusers/list`)
  ...
  const columns = [
...

    {
      ...
      cell: (row) => {
        return (
          ...
          >
            {row.cleartext_password ? row.cleartext_password : "---"}
          </span>
        );
      },
      sortable: false,
    ...
    {
      name: <b className="Table_columns adminuser_columns">{"Is active"}</b>,
      selector: "is_active",
      // width: '10%',
      // width: 'auto',
    ...
    {
      name: <b className="Table_columns adminuser_columns">{"Plan Type"}</b>,
      ...
      cell: (row) => (
        ...
        >
          {" "}
          {row.service_plan ? row.service_plan : "---"}
        </span>
      ),
      sortable: false,
    },
                            ...
                            {activeTab1 == "3" && (
                              <Userdetails
                                lead={{
                                  ...
                                  cleartext_password: lead.cleartext_password,
                                  service_plan: planList.filter(
                                    (pro) =>
                                      pro.package_name === lead.service_plan
                                  )[0].id,
                                }}
