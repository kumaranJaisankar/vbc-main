...
const LeadDateRange = (props) => {
 
  //handler for custom date
  const customHandler = (e) => {
    if (e.target.name === "start_date") {
      props.setLeadcustomstartdate(e.target.value);
      
    }
    if (e.target.name === "end_date") {
      props.setLeadCustomenddate(e.target.value);
    }
  };
  ...
  return (
    <>
      <div className="input_wrap" style={{ position: "relative", top: "6px",left:"7%" }}>
        <Input
          className={`form-control-digits not-empty`}
          type="select"
          name=""
          value={props.leadDaterange}
          onChange={props.leadDaterangeselection}
          style={{
            width: "fit-content",
